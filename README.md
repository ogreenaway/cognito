...
If I had more time I would work closely with a designer to create a design system with tokens and a colour pallet. We could then encorperate that into a Bootstrap theme or create our own SCSS framework from scratch.
...
The topic, subtopic, and LLM calls need to be made sequentially and so I created a custom hook (useTopicData) with a async function to try to increase readibility.
...
If I had more time I would add more testing for each API failing to check the Error Boundary and a useful message is always shown. I would also work with a backend engineer to try to update the GraphQL to allow getting all the subtopics in one API call. The apps latency could be greatly reduced if we could merge the concurrent API calls.
...
As explained at the end of the Loom, I gave the LLM all the context it needed to create the categories. I used a formatted schema for its response becuase I need an array of objects I could parse and display, not just text. To make the API call quicker, and cheaper, I minimised the amount of info it needed to return, down to just the minimum IDs.

I picked a cheaper and faster model to help speed up the API and I created a dynamic loading state to show the progress.
...
I ran the API calls in parallel
...
If I had more time I would add a storybook of all the reusuable components. This is useful for designers to review but also helps engineers build new features and get early feedback.
